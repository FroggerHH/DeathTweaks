...
public class ConfigSync
{
    ...
    private void RPC_FromServerConfigSync(ZRpc rpc, ZPackage package)
    {
        ...
        if (HandleConfigSyncRPC(0, package, false)) InitialSyncDone = true;
    }
    ...
    private bool HandleConfigSyncRPC(long sender, ZPackage package, bool clientUpdate)
    {
        try
        {
            if (isServer && IsLocked && SnatchCurrentlyHandlingRPC.currentRpc?.GetSocket()?.GetHostName() is { } client)
            {
                ...
                var exempt = listContainsId is null
            ...
            if ((packageFlags & FRAGMENTED_CONFIG) != 0)
            {
                ...
                if (dataFragments.Count < fragments) return false;
            ...
            var originalSaveOnConfigSet = false;
            ...
            if (configFile is not null) configFile.SaveOnConfigSet = originalSaveOnConfigSet;
            ...
            if (!isServer) serverLockedSettingChanged(); // Re-evaluate for intial locking
    ...
    private ParsedConfigs ReadConfigsFromPackage(ZPackage package)
    {
        ...
        var configMap = allConfigs.Where(c => c.SynchronizedConfig)
    ...
    {
        if (configSyncs.FirstOrDefault(cs => cs.allConfigs.Contains(config)) is not { } configSync) return true;
    ...
    private static ZPackage ConfigsToPackage(IEnumerable<ConfigEntryBase>? configs = null,
        ...
        bool partial = true)
    {
        var configList = configs?.Where(config => configData(config)!.SynchronizedConfig).ToList()
    ...
    private static object ReadValueWithTypeFromZPackage(ZPackage package, Type type)
    {
        ...
        if (type != typeof(List<string>) && type.IsGenericType
                                         && typeof(ICollection<>).MakeGenericType(type.GenericTypeArguments[0]) is
                                             { } collectionType && collectionType.IsAssignableFrom(type))
        {
            var entriesCount = package.ReadInt();
            ...
            IEnumerator WatchAdminListChanges()
            {
                ...
                for (;;)
                {
                    ...
                    if (!adminList.GetList().SequenceEqual(CurrentList))
                    {
                        ...
                        {
                            var client = p.m_rpc.GetSocket().GetHostName();
                            return listContainsId is null
                                ? adminList.Contains(client)
                                : (bool)listContainsId.Invoke(ZNet.instance, new object[] { adminList, client });
                        }).ToList();
        ...
        private static void Postfix(Dictionary<Assembly, BufferingSocket> __state, ZNet __instance, ZRpc rpc)
        {
            ...
            IEnumerator sendAsync()
            {
                foreach (var configSync in configSyncs)
                {
                    ...
                    var listContainsId = AccessTools.DeclaredMethod(typeof(ZNet), "ListContainsId");
                    var adminList = (SyncedList)AccessTools.DeclaredField(typeof(ZNet), "m_adminList")
                    ...
                    var package = ConfigsToPackage(configSync.allConfigs.Select(c => c.BaseConfig),
    ...
    private static class PreventSavingServerInfo
    {
        ...
        {
            if (configData(__instance) is not { } data || isWritableConfig(data)) return true;
    ...
    private static class PreventConfigRereadChangingValues
    {
        ...
        {
            if (configData(__instance) is not { } data || data.LocalBaseValue == null) return true;
...
public class VersionCheck
{
    ...
    private static void PatchServerSync()
    {
        if (PatchProcessor.GetPatchInfo(AccessTools.DeclaredMethod(typeof(ZNet), "Awake"))?.Postfixes
                .Count(p => p.PatchMethod.DeclaringType == typeof(ConfigSync.RegisterRPCPatch)) > 0) return;
        ...
        foreach (var type in typeof(ConfigSync).GetNestedTypes(BindingFlags.NonPublic)
                     .Concat(new[] { typeof(VersionCheck) }).Where(t => t.IsClass)) harmony.PatchAll(type);
    }
    ...
    {
        if (ReceivedMinimumRequiredVersion == null) return $"{DisplayName} is not installed on the server.";
        var myVersionOk = new System.Version(CurrentVersion) >= new System.Version(ReceivedMinimumRequiredVersion);
        return myVersionOk
            ? $"{DisplayName} may not be higher than version {ReceivedCurrentVersion}. You have version {CurrentVersion}."
            : $"{DisplayName} needs to be at least version {ReceivedMinimumRequiredVersion}. You have version {CurrentVersion}.";
    }
    ...
    private static void ShowConnectionError(FejdStartup __instance)
    {
        if (!__instance.m_connectionFailedPanel.activeSelf
            || ZNet.GetConnectionStatus() != ZNet.ConnectionStatus.ErrorVersion) return;
        ...
        if (failedCheck)
        {
            var panel = __instance.m_connectionFailedPanel.transform.Find("Image").GetComponent<RectTransform>();
            panel.sizeDelta = panel.sizeDelta with { x = 675 };
            __instance.m_connectionFailedError.ForceMeshUpdate();
            var newHeight = __instance.m_connectionFailedError.renderedHeight + 105;
            var button = panel.transform.Find("ButtonOk").GetComponent<RectTransform>();
            button.anchoredPosition = new Vector2(button.anchoredPosition.x,
                button.anchoredPosition.y - (newHeight - panel.sizeDelta.y) / 2);
